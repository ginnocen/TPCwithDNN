...
def nn_1d(config, inputs, exp_outputs):
    ...
    for _ in range(config.nn_params["n_hidden_layers"]):
        model.add(Dense(units=config.nn_params["n_neurons"]*inputs.shape[1],
                        activation=config.nn_params['hidden_activation']))
...
def nn_1d_with_validation(config, inputs, exp_outputs, inputs_val, outputs_val):
    ...
    :rtype: keras.model
    """
    if not os.path.isdir("%s/learning_plot_%s_nEv%d" %
                         (config.dirplots, config.suffix, config.train_events)):
        os.makedirs("%s/learning_plot_%s_nEv%d" %
                    (config.dirplots, config.suffix, config.train_events))
    ...
    plt.savefig("%s/learning_plot_%s_nEv%d/training_loss.eps" %
                (config.dirplots, config.suffix, config.train_events))
    plt.savefig("%s/learning_plot_%s_nEv%d/training_loss.pdf" %
                (config.dirplots, config.suffix, config.train_events))
